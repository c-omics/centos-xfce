...
function start_vnc {
  ...
  if [[ ! -z ${VNCPASSWORD+x} ]] ; then
    ...
    su -l $DEFAULT_USER -c "export PATH=$PATH ; x11vnc -xvnc -env FD_PROG=/usr/bin/xfce4-session -gone 'killall xfce4' -bg -rfbauth ~/vnc.pass"
  else
    su -l $DEFAULT_USER -c "export PATH=$PATH ;  x11vnc -xvnc -env FD_PROG=/usr/bin/xfce4-session -gone 'killall xfce4' -bg -nopw"
  fi
}
